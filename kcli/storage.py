...
class Storage:
    ...
    def add(self, doc: Document):
        ...
        cursor.execute(
            ...
            (
                ...
                json.dumps(np.array(doc.embedding).tolist()) if doc.embedding is not None else None,
                str(doc.meta),
            ),
        )
    ...
    def _brut_force_search(self, query: str, limit: int = 10, similarity_threshold = None) -> List[Document]:
    ...
    def search(self, query: str, limit: int = 10, similarity_threshold = None) -> List[Document]:
    ...
    def _hnsw_search(self, query: str, limit: int = 10, similarity_threshold = None) -> List[Document]:
        ...
        try:
            ...
            if similarity_threshold is not None:
                filtered_results = [
                    label for label, distance in zip(labels[0], distances[0])
                    if 1 - distance >= similarity_threshold
                ]
            else:
                filtered_results = labels[0]
            doc_ids = filtered_results.tolist()[0]
